...
#دانلود#
  ...
  try:
   if url.find('/'):
    ...
    app.send_document(m.chat.id , f"{filename}" , caption=f"""**آپلود**\n✿ اسم : `{filename}`\n✿ حجم : `{total/1024/1024:.3f} ᴍʙ`\n✿ ساعت : `{datetime.now(timezone("Asia/Tehran")).strftime("%H:%M")}`/n SELf X""")
    app.delete_messages(m.chat.id , m.id)
    os.remove(filename)
  except:
...
✿ `CPU` ~  (`{cpu_percent()}%`)""")
  ...
  try:
    app.edit_message_text(m.chat.id , m.id , f"""
**🧸به بخش راهنمای سلف X خوش اومدی 🧸**
...
""")
  ...
  try:
    app.edit_message_text(m.chat.id , m.id , f"""
**به بخش تنظيمات سلف X خوش اومدی ⚙**
`ping` | `پینگ`
...
""")
  ...
  try:
    app.edit_message_text(m.chat.id , m.id , f"""
**به بخش سرگرمی سلف X خوش اومدی 🎪**

----------------------------
جنبه شوخی و کاربردی ایستگاه 😂🩷 :

`اد خایمال` | `addkhaymal`
----------------------------
...
""")
  ...
  try:
    app.edit_message_text(m.chat.id , m.id , f"""
**به بخش مود منو سلف X خوش اومدی 🔋**
...
""")
  ...
  try:
    app.edit_message_text(m.chat.id , m.id , f"""
*به منو کاربری سلف X خوش اومدید 💣*
...
#بخش گیم های رندوم#
  ...
  m.edit_text(f"شما خایمال شناسایی شدی 😐😄") 

 elif text.startswith("اد خایمال"):
  m.edit_text(f"شما خایمال شناسایی شدی 😐😄") 
